...
#ifdef __cplusplus
        std::sort((double*)ptrs[i][0], (double*)ptrs[i][0] + n);
#else
...
{                                                                   
    ...
    for(j = 4; j < jend; j++) {                                     
       ...
       /* printf("[dbg] breakpoint at %s %d\n" , __FILE__ , __LINE__ );
        printf("[dbg] dumping out the array for outlen=%d\n", outlen );
        int64_t* _start = (int64_t *) ptrs[ptridx ^ 1]; 
        for (int i = 0; i < BLOCK_SIZE; ++i)
        {
        	if( i != 0 && i % outlen == 0) puts("");
            printf("[dbg] index: %d \t\t value: %d\n", i,  GetKeyInt(*(_start + i)) ); 
        } 
        printf("[dbg] examine the output array for outlen=%d\n", outlen );
        for (int i = 0; i < BLOCK_SIZE; i += outlen)
        {
        	for (int z = i; z < i + outlen - 1; ++z)
        	{
        		if( GetKeyInt(*(_start + z))  <= GetKeyInt(*(_start + z + 1)) ) continue; 
        		else {
        			printf("[dbg] violation found at %d\n", z );
        			printf("[dbg] index: %d = %d\n", z , GetKeyInt(*(_start + z)) );
        			printf("[dbg] index: %d = %d\n", z+1 , GetKeyInt(*(_start + z +1 ) ) );
        		}
        	}
        }*/
    }
